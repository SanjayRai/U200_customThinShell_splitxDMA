...
bool fpga_CDMA_XFER (fpga_dev_mem *fpga_AXI_Lite_ptr, uint64_t SRC_ADDR, uint64_t DEST_ADDR, uint32_t XFER_SZ ) {
    ...
    while (ret_data != 0x00001002) {
        ret_data = (fpga_AXI_Lite_ptr->fpga_peek(CDMA_STATUS_REG) & 0x00001002); 
        itn_count++; 
        if (itn_count > 10000000)  {
...
void fpga_test_AXIL_LITE_8KSCRATCHPAD_BRAM_VF1 (fpga_dev_mem *fpga_AXI_Lite_ptr) {

    cout << "Testing Axi_Lite ScratchPad BRAM\n";
    uint32_t tmp_dbg_val[8192];
    for (int idx = 0; idx < 8192; idx+=4) {
        tmp_dbg_val[idx]  = (uint32_t)(rand() % 0xffffffff); 
        fpga_AXI_Lite_ptr->fpga_poke((AXI_LITE_SCRATCH_PAD_BRAM_VF1 + idx), tmp_dbg_val[idx]);
    }
    uint32_t tmp_dbg;
    for (int idx = 0; idx < 8192; idx+=4) {
        tmp_dbg= fpga_AXI_Lite_ptr->fpga_peek(AXI_LITE_SCRATCH_PAD_BRAM_VF1 + idx);
        if (tmp_dbg != tmp_dbg_val[idx]) {
            printf("Error !!! Axi_lite Test failed @ Index = %d :: Expected %x but got %x\n", idx, tmp_dbg_val[idx], tmp_dbg);
        }
    }
...
void fpga_PROGRAM_NORTH_PR (fpga_dev_mem *fpga_AXI_Lite_ptr, string PR_binFile_name) {
    ...
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_GPIO_PR_ISOLATION, ISOLATE_NORTH_PR); 
    ICAP_prog_PR_binfile (fpga_AXI_Lite_ptr, PR_binFile_name);
   cout << "PR_BIN_FILE_NAME = " << PR_binFile_name << endl; 
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_GPIO_PR_ISOLATION, DEISOLATE_NORTH_PR); 
}

void fpga_PR_DEISOLATE (fpga_dev_mem *fpga_AXI_Lite_ptr) {
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_GPIO_PR_ISOLATION, DEISOLATE_NORTH_PR); 
}

void fpga_PR_ISOLATE (fpga_dev_mem *fpga_AXI_Lite_ptr) {
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_GPIO_PR_ISOLATION, ISOLATE_NORTH_PR); 
}
...
void assert_DDR_rst_all(fpga_dev_mem *fpga_AXI_Lite_ptr) {
    unsigned short  RESET_REG;
    RESET_REG = fpga_AXI_Lite_ptr->fpga_peek(AXI_LITE_SHELL_TO_CL_RESET_STAT); 
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_SHELL_TO_CL_RESET, (RESET_REG | 0x3)); 
}

void deassert_DDR_rst_all(fpga_dev_mem *fpga_AXI_Lite_ptr) {
    unsigned short  RESET_REG;
    RESET_REG = fpga_AXI_Lite_ptr->fpga_peek(AXI_LITE_SHELL_TO_CL_RESET_STAT); 
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_SHELL_TO_CL_RESET, (RESET_REG & 0x8)); 
}
...
void deassert_CL_rst_main_n (fpga_dev_mem *fpga_AXI_Lite_ptr) {
    unsigned short  RESET_REG;
    RESET_REG = fpga_AXI_Lite_ptr->fpga_peek(AXI_LITE_SHELL_TO_CL_RESET_STAT); 
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_SHELL_TO_CL_RESET, (RESET_REG & 0x7)); 
}

void set_ddr4_RESET_GATE (fpga_dev_mem *fpga_AXI_Lite_ptr) {
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_RESET_GATE, 0x1); 
}

void unset_ddr4_RESET_GATE (fpga_dev_mem *fpga_AXI_Lite_ptr) {
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_RESET_GATE, 0x0); 
}
...
uint64_t wait_ddr_CAL_DONE (fpga_dev_mem *fpga_AXI_Lite_ptr, uint32_t DDR_INTERFACE) {
    ...
    while (RESET_REG != ddr_mask) {
        ...
        if (itn_count > 50000)  {
            RESET_REG = (fpga_AXI_Lite_ptr->fpga_peek(AXI_LITE_ROLE_GPIO_MIG_CAL_STAT) & ddr_mask);
            cout << "SREF Error :: DDR Interface  mask  " <<  hex << ddr_mask << " did not complete calibration" <<  " Status = " << fpga_AXI_Lite_ptr->fpga_peek(AXI_LITE_ROLE_GPIO_MIG_CAL_STAT) << endl;;
            break;
        }
    }
    return (itn_count);
...
void fpga_ICAP_RESET_CRC_VALUE (fpga_dev_mem *fpga_AXI_Lite_ptr) {
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_ICAP_CRC_BASE, 0x1);
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_ICAP_CRC_BASE, 0x0);
}
uint32_t fpga_ICAP_READ_CRC_VALUE (fpga_dev_mem *fpga_AXI_Lite_ptr) {
    return fpga_AXI_Lite_ptr->fpga_peek(AXI_LITE_ICAP_CRC_BASE + 0x8);
}
